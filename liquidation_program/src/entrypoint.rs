...
use LiquidationError::InvalidInstruction;
use spl_token::{
    solana_program::{
...
pub enum LiquidationInstruction {
...
impl Processor {
    pub fn process(
        ...
        let instruction = LiquidationInstruction::unpack(instruction_data)?;

        match instruction {
            LiquidationInstruction::LiquidateObligation { amount } => {
                msg!("Instruction: Liquidate obligation");
                Self::process_liquidate_obligation(accounts, amount, program_id)
            }
        }
    }
...
impl From<LiquidationError> for ProgramError {
    fn from(e: LiquidationError) -> Self {
